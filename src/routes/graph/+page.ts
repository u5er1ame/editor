	...
	for (const table of node_tables) {
		...
		if (val && val.length > 0) {
			...
			if (meta.graph.type == 'edge') {
				error(
					422,
					'Edge type config in node-like table please check table type and/or config validity'
				);
			}
		...
		meta = addFieldsMetadata(schemas.client, false, table.name);
		baseConfigStore.addViewConfig(schemas.client, meta);
	...
	for (const table of edge_tables) {
		...
		if (val && val.length > 0) {
			...
			if (meta.graph.type == 'node') {
				error(
					422,
					'Node type config in edge-like table please check table type and/or config validity'
				);
			}
			...
			val.forEach((item) => {
				...
				if (in_parent && out_parent && in_parent == out_parent) {
					edgeTypes.inbound = Edges.Inbound;
					meta.graph.flowConfig.type = 'inbound';
				} else {
					edgeTypes.outbound = Edges.Outbound;
					meta.graph.flowConfig.type = 'outbound';
				}
