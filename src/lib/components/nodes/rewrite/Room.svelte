<script module lang="ts">
	export type Props = NodeProps<Node<{ raw: ElectricRoom; labelKey: keyof ElectricRoom }>> &
...
<script lang="ts">
	...
	let selected = $derived.by(
		() => selectedNodes.length > 0 && selectedNodes.every((item) => item == id)
	);
	...
	function ondblclick(e: MouseEvent) {
		...
		if (!node) return;
		if (node.selectable) {
			flow.fitBounds(flow.getNodesBounds([node]), { padding: 0.15 });
		} else {
	...
	const onfocus = (e: FocusEvent & { currentTarget: EventTarget & HTMLInputElement }) =>
...
{#if browser}
	<NodeResizer
		{...resizeProps}
		isVisible={selected && resizeable}
		color="var(--color-orange-400)"
		lineClass=""
		nodeId={id}
	/>
	<p {ondblclick} class="flex-1 content-center p-2 text-foreground opacity-30">{name}</p>
	<EditToolbar
		isVisible={selected && zoom}
		bind:editable={editName}
		{id}
		size={s.toString() + 'px'}
	/>
	{#if zoom}
		...
		>
			{#if editName}
				...
				<input
					...
					{onfocus}
					class="w-full bg-transparent text-lg text-foreground focus:bg-hover"
					bind:value={name}
					{onblur}
				/>
			{:else}
	...
	<NodeToolbar
		...
		nodeId={id}
	>
		<p class="size-auto font-extralight italic">{data?.raw?.id}</p>
	</NodeToolbar>
{/if}
...
<style>
	...
	:global(.svelte-flow__node-electric_rooms:not(.draggable)) {
		...
		/* text-align: center; */
		border: 2px dashed --alpha(var(--color-hover) / 30%);
		backdrop-filter: blur(2px);
	}
</style>
