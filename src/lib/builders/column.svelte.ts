...
export class ColumnBuilder {
	...
	static hidden(id: string) {
		return new ColumnBuilder(id).hidden().build();
	}
	static default(id: string) {
		return new ColumnBuilder(id).defaultHeader().grow().sort().build();
	}
	static defaultWithKey(id: string, key: string) {
		return new ColumnBuilder(id).defaultHeader().grow().sort(sortByProperty(key)).template((val)=>val[key]??"ERROR").build();
	}
}
