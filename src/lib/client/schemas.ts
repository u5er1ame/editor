...
const LevelSchema = z.object({
	...
	name: z.string().meta({ column: { editor: "text", flexgrow: 1 } }),
});
const ElectricRoomSchema = z.object({
	id: z.custom<RecordId<"electric_rooms">>().meta({ column: { hidden: true } }),
	name: z.string().meta({ column: { sort: true, editor: "text", auto: "data" } }),
	level: z.custom<RecordId<"levels">>().meta({ table: "levels", filter: "richselect", column: { sort: true, editor: "richselect" } }),
})
const BoardSchema = z.object({
	id: z.custom<RecordId<"boards">>().meta({ hidden: true }),
	name: z.string().meta({ column: { editor: "text", auto: "data" } }),
	room: z.custom<RecordId<"electric_rooms">>().meta({ table: "electric_rooms", column: { editor: "combo", auto: "data" } }),
})
...
const AreaNameSchema = z.object({
	...
	name: z.string().meta({ column: { header: [{ text: "Name" }, { filter: "text" }], sort: true, editor: "text", auto: "data" } }),
	shop: z.custom<RecordId<"shops">>().optional().meta({
		table: "shops",
		filter: "text",
		column: {
...
const ShopSchema = z.object({
	...
	name: z.string().meta({ column: { editor: "text", flexgrow: 1, sort: (a: any, b: any) => a.name.localeCompare(b.name) } }),
});
