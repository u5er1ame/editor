...
const LevelSchema = z.object({
	...
	name: z.string(),
}).register(table_registry, { id: "level", title: "Level" });
...
export const schemas: Schemas[] = [BreakerConnectionSchema, LevelSchema, ElectricRoomSchema, BoardSchema, BreakerSchema, AreaNameSchema, ShopSchema];
export const tables: Table[] = schemas.map((schema: Schemas) => { return new Table(table_registry.get(schema)!.id) });
