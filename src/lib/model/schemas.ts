...
export const ShopSchema = z.object({
	...
	name: z.string(),
	area_name: z.custom<RecordId<'area_name'>>().optional()
});
export const ClientShopSchema = ShopSchema.extend({
	area_name: AreaNameSchema
});
const ShopQuery = new BoundQuery('select * from shops fetch area_name.*');
...
const BreakerQuery = new BoundQuery(
	'select * from breakers fetch board, board.room, board.room.level'
);
...
export type EdgesSchemas = typeof BreakerConnectionSchema;
